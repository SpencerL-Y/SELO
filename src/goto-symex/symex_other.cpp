...
void goto_symext::symex_dead(const expr2tc code)
{
  ...
  if (options.get_bool_option("z3-slhv"))
  {
    // Call free on alloca'd objects
    if (
      identifier.as_string().find("return_value$__builtin_alloca") !=
      std::string::npos)
      symex_free(code_free2tc(l1_sym));
    ...
    if (is_struct_type(code2))
    {
      ...
      for (it = cur_state->top().local_heap_regions.begin();
           it != cur_state->top().local_heap_regions.end();
           ++it)
      {
        expr2tc l1_reg = *it;
        cur_state->top().level1.get_ident_name(l1_reg);
        if (
          to_symbol2t(l1_sym).get_symbol_name() ==
      ...
      {
        log_error(
          "Do not record this stack struct ? - {}", identifier.as_string());
        abort();
      }

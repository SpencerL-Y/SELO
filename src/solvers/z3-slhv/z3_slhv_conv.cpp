...
smt_astt z3_slhv_convt::mk_pt(smt_astt l, smt_astt v)
{
  ...
  return new_ast(
    z3::points_to(
      to_solver_smt_ast<z3_smt_ast>(l)->a, to_solver_smt_ast<z3_smt_ast>(v)->a),
...
smt_astt z3_slhv_convt::mk_locadd(smt_astt l, smt_astt o)
{
  ...
  return new_ast(
    z3::locadd(
      to_solver_smt_ast<z3_smt_ast>(l)->a, to_solver_smt_ast<z3_smt_ast>(o)->a),
    this->mk_intloc_sort());
}
...
smt_astt z3_slhv_convt::mk_loc2int(smt_astt l)
{
  assert(l->sort->id == SMT_SORT_INTLOC);
  return new_ast(
    z3::loc2int(to_solver_smt_ast<z3_smt_ast>(l)->a), this->mk_int_sort());
}
...
smt_astt z3_slhv_convt::convert_slhv_opts(
  const expr2tc &expr,
  const std::vector<smt_astt> &args)
{
  ...
  case expr2t::heap_region_id:
  {
    ...
    if (_type.is_aligned)
    {
      for (unsigned int i = 0; i < _type.field_types.size(); i++)
      {
        smt_astt loc =
          i == 0 ? base_loc : mk_locadd(base_loc, mk_smt_int(BigInt(i)));
        smt_sortt sort = is_pointer_type(_type.field_types[i])
                           ? mk_intloc_sort()
                           : mk_int_sort();
        std::string name = mk_fresh_name(
          is_pointer_type(_type.field_types[i]) ? "tmp_loc::" : "tmp_val::");
    ...
    else
    {
      ...
      pt_vec.push_back(mk_pt(
        base_loc, mk_fresh(mk_intloc_sort(), mk_fresh_name("tmp_loc::"))));
    }
    return pt_vec.size() == 1 ? pt_vec[0] : mk_uplus(pt_vec);
  }
  case expr2t::location_of_id:
  {
    ...
    if (is_intheap_type(locof.source_heap))
    {
      ...
      is_symbol2t(locof.source_heap) &&
      (is_intloc_type(locof.source_heap) || is_scalar_type(locof.source_heap)))
    {
      std::string loc_name =
    ...
    else if (is_field_of2t(locof.source_heap))
    {
      ...
      if (
        !is_intheap_type(fieldof.source_heap->type) ||
      ...
      loc =
        mk_locadd(convert_ast(_type.location), convert_ast(fieldof.operand));
    }
    else
    {
  ...
  case expr2t::field_of_id:
  {
    ...
    if (is_constant_intheap2t(field_of.source_heap))
      return mk_fresh(
        convert_sort(field_of.type), mk_fresh_name("invalid_fieldof_"));
    ...
    smt_astt l =
      _field == 0 ? source_loc : mk_locadd(source_loc, convert_ast(field));
  ...
  case expr2t::heap_delete_id:
  {
    ...
    smt_astt h1 = mk_fresh(h->sort, mk_fresh_name(std::string("_tmp_heap_")));
    smt_astt v1 =
      mk_fresh(mk_int_sort(), mk_fresh_name(std::string("_tmp_val_")));

    smt_astt nl =
      mk_fresh(mk_intloc_sort(), mk_fresh_name(std::string("_assigned_loc_")));
    smt_astt heap_state =
